...
def generate_pdf_bill(sale_number, cart, purchasetime, customer_name, payment_method, customer_phone, customer_address):
    ...
    logo_path = "C:/Users/Kishore/Desktop/billingsys/env/Scripts/billingsystemapp/templates/static/png2.png"  # Adjust path as per your project structure
    ...
    for item in cart.values():
     ...
     data.append([
        ...
        item['name'],
        item['quantity'],
        f"Rs {round(item['original_price'], 2)}",
        f"Rs {round(item['discounted_price'], 2)}",
        f"Rs {total_price}"
    ])
     serial_number += 1 
